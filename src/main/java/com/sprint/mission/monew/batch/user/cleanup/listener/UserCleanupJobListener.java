...
public class UserCleanupJobListener implements JobExecutionListener {
  ...
  public void afterJob(JobExecution jobExecution) {
    ...
    if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
      log.info("User Cleanup Job 성공 | jobId={}", jobExecution.getId());
      userCleanupMetrics.markSuccess();
    }
    ...
    if (jobExecution.getStartTime() != null && jobExecution.getEndTime() != null) {
      ...
      userCleanupMetrics.recordJobDuration(duration);

      log.info("User Cleanup Job duration={}", duration);
    } else {
      log.warn("User Cleanup Job 시간 정보 누락 | start={}, end={}",
          jobExecution.getStartTime(),
          jobExecution.getEndTime());
    }
