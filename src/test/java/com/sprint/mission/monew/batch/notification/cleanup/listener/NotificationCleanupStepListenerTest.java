...
public class NotificationCleanupStepListenerTest {
  ...
  @InjectMocks
  NotificationCleanupStepListener listener;
  ...
  void metrics_실패해도_Step_정상_종료() {
    ...
    doThrow(new RuntimeException("metrics fail")).when(notificationMetrics)
    ...
    // then
    then(notificationMetrics).should().countDeleted(123L);
    assertThat(result).isEqualTo(ExitStatus.COMPLETED);
  }
  ...
  void 스텝_실행_후_메트릭스가_기록된다() {
    ...
    // then
    then(notificationMetrics).should().countDeleted(123);
    then(notificationMetrics).should().recordStepDuration(Duration.ofSeconds(5));
    assertThat(result).isEqualTo(ExitStatus.COMPLETED);
  }
}