...
public class NotificationCleanupJobListenerTest {
  ...
  @InjectMocks
  NotificationCleanupJobListener listener;
  ...
  class AfterJob {
    ...
    void job이_실패하여_FailureException_있으면_warn로그_실행() {
      ...
      // then
      verify(notificationMetrics, never()).markSuccess();
    }
    ...
    void job_성공하면_markSuccess_호출() {
      ...
      // then
      verify(notificationMetrics, times(1)).markSuccess();
      verify(notificationMetrics).recordJobDuration(Duration.ofSeconds(5));
