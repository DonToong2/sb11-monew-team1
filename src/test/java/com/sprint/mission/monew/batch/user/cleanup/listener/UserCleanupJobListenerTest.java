...
public class UserCleanupJobListenerTest {
  ...
  class AfterJob {
    ...
    void 시간_null이면_metrics_미호출() {
      ...
      // then
      then(userCleanupMetrics).should(never()).recordJobDuration(Duration.ofSeconds(anyLong()));
    }
    ...
    void job이_실패하여_FailureException_있으면_warn로그_실행() {
      ...
      // then
      verify(userCleanupMetrics, never()).markSuccess();
    }
    ...
    void job_성공하면_markSuccess_호출() {
      ...
      // then
      verify(userCleanupMetrics, times(1)).markSuccess();
      verify(userCleanupMetrics).recordJobDuration(Duration.ofSeconds(5));
    }
  }
}
